    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="px-4 h-14 flex items-center justify-between">
          ...
          {/* Logo */}
          <Link to="/" className="flex flex-col items-start leading-none" onClick={() => setDrawerOpen(false)}>
            <span className="font-display text-2xl italic text-charcoal tracking-wide leading-none">Glamour</span>
            <span className="font-sans text-[10px] tracking-[0.4em] text-rose uppercase ml-8 leading-none mt-0.5">Boutique</span>
          </Link>
          ...
          <div className="flex items-center gap-0.5">
            ...
            <button onClick={() => setSearchOpen(s => !s)}
              className="w-10 h-10 flex items-center justify-center text-charcoal hover:text-rose transition-colors"
              aria-label="Search">
            ...
            <Link to="/favourites"
              className="w-10 h-10 items-center justify-center relative text-charcoal hover:text-rose transition-colors hidden md:flex"
              aria-label="Favourites">
              <Heart size={20} />
              {favourites.length > 0 && (
                <span className="absolute top-1.5 right-1 w-3.5 h-3.5 bg-rose text-white text-[9px] flex items-center justify-center rounded-full leading-none">
                  {favourites.length}
                </span>
              )}
            </Link>
      ...
      {user ? (
        ...
        <Link to="/login" className="block px-3 py-2.5 text-xs font-sans hover:bg-champagne">Sign In</Link>
            ...
            <button
              ...
              className="w-10 h-10 flex items-center justify-center text-charcoal md:hidden"
        ...
        <div className="hidden md:flex items-center justify-center gap-8 pb-3 border-b border-gray-100">
          {[{ to: '/', label: 'Home' }, { to: '/contact', label: 'Contact' }].map(l => (
            <NavLink key={l.to} to={l.to} className={({ isActive }) =>
              `font-sans text-[11px] tracking-[0.15em] uppercase transition-colors ${isActive ? 'text-rose' : 'text-charcoal hover:text-rose'}`
            }>{l.label}</NavLink>
          ))}
...
<div className="relative group">
  ...
  {/* pt-3 creates invisible hover bridge over the gap */}
  <div className="absolute top-full left-1/2 -translate-x-1/2 pt-3 w-[480px] opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-opacity duration-200 z-50">
    <div className="bg-ivory border border-gray-100 shadow-2xl p-6">
          ...
          <div className="border-t border-gray-100 bg-ivory px-4 py-2.5">
            <form onSubmit={handleSearch} className="flex gap-2 max-w-lg mx-auto">
              <input
                ...
                className="flex-1 border border-gray-200 px-3 py-2.5 text-sm font-sans text-charcoal bg-white focus:outline-none focus:border-rose"
              />
              <button type="submit"
                className="bg-charcoal text-ivory px-4 py-2.5 text-xs font-sans tracking-widest uppercase hover:bg-rose transition-colors whitespace-nowrap">
                Go
              </button>
            </form>
          </div>
      ...
      {drawerOpen && (
        <>
          <div className="fixed inset-0 z-[60] bg-charcoal/40 backdrop-blur-sm md:hidden"
            onClick={() => setDrawerOpen(false)} />

          <div className="fixed top-0 left-0 bottom-0 z-[70] w-[85vw] max-w-sm bg-ivory flex flex-col md:hidden overflow-y-auto"
            style={{ transform: 'translateX(0)', transition: 'transform 0.3s ease' }}>

            {/* Drawer header */}
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100">
              <div>
                <span className="font-display text-2xl italic text-charcoal leading-none">Glamour</span>
                <span className="block font-sans text-[8px] tracking-[0.4em] text-rose uppercase mt-0.5">Boutique</span>
              </div>
            ...
            {user ? (
              ...
              <div className="px-5 py-4 border-b border-gray-100">
                <Link to="/login" onClick={() => setDrawerOpen(false)}
                  className="block w-full text-center bg-charcoal text-ivory py-3 text-xs font-sans tracking-widest uppercase">
                  Sign In / Register
                </Link>
              </div>
            )}
            ...
            <div className="flex-1 px-5 py-4">
              <div className="space-y-1">
                {[
                  { to: '/',            label: 'Home' },
                  { to: '/favourites',  label: `Favourites${favourites.length ? ` (${favourites.length})` : ''}` },
              ...
              {/* Collections grid */}
              <div className="mt-5">
                <p className="font-sans text-[9px] tracking-[0.25em] uppercase text-mink mb-3">Collections</p>
                <div className="grid grid-cols-2 gap-2">
                  {categories.map(cat => (
                    <Link key={cat} to={`/collections/${cat.toLowerCase()}`}
                      onClick={() => setDrawerOpen(false)}
                      className="bg-champagne/50 px-3 py-3 flex items-center justify-between active:bg-champagne">
                      <span className="font-display text-base text-charcoal">{cat}</span>
            ...
            {user && (
              <div className="px-5 py-4 border-t border-gray-100">
                <button onClick={handleLogout}
                  className="text-xs font-sans text-mink hover:text-rose transition-colors">
                  Sign Out
                </button>
              </div>
            )}
